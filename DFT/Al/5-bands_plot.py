...
def bndplot(datafile, fermi, symmetryfile, subplot, **kwargs):
    ...
    bands = []
    # This gives the number of bands in the calculation
    bndl = len(z[z[:, 0] == x[1]])
    Fermi = float(fermi)
    ...
    for i in range(0, bndl):
        # This is where we storre the bands
    ...
    for i in bands:  # Here we plots the bands
        subplot.plot(i[:, 0], i[:, 1]-fermi_shift,
                     color=color_bnd, linestyle=line_bnd)
    if 'legend' in kwargs:
        # empty plot to generate legend
        subplot.plot([None], [None], color=color_bnd,
                     linestyle=line_bnd, label=kwargs['legend'])
    ...
    subplot.plot([min(x), max(x)], [Fermi-fermi_shift, Fermi -
                                    fermi_shift], color='red', linestyle='dotted')
